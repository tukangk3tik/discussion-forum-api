...
describe('ReplyRepositoryPostgres', () => {
  ...
  describe('addReply function', () => {
    ...
    it('should persist add new reply', async () => {
      ...
      // Assert
      const addedReply =
          await ReplyTableTestHelper.getReplyByContent('Reply for comment 123');
  ...
  describe('getReply by Comment Id function', () => {
    it('should get reply by comment id', async () => {
      ...
      // Action
      const getReplies = await replyRepositoryPostgres.getReplyByCommentIds(id);

      // Assert
      expect(getReplies.length).toEqual(1);
      expect(getReplies[0].id).toEqual('reply-56789');
      expect(getReplies[0].content).toEqual('Reply for comment 123');
      expect(getReplies[0].comment_id).toEqual('comment-8932');
      expect(getReplies[0].username).toEqual('test-user');
      expect(getReplies[0].date).toBeDefined();
    });
  });
  ...
  describe('deleteReply function', () => {
    ...
    it('should get deleted reply by comment id ', async () => {
      // Arrange
      const commentId = ['comment-8932'];
      ...
      // Action
      const getReplies =
          await replyRepositoryPostgres.getReplyByCommentIds(commentId);
  ...
  describe('verifyOwner function', () => {
    ...
    it('should throw authorization error', async () => {
      ...
      await expect(replyRepositoryPostgres
          .verifyOwner('reply-37973', 'user-123456'))
          .rejects
